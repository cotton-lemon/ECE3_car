...
int Kp=60;//P will be devided by Kp
float Kd=1/4;//0.5*2
//global
int leftSpd = base_speed;
int rightSpd = base_speed;
int preError[3]={0,0,0};
int preDelta[3]={0,0,0};
...
void setup() {
  ...
  //   preError[i]=tmp;
  // }
  int Kp=46;//P will be devided by Kp
  float Kd=1/4;//0.5*2
  starttime=millis();
...
if (millis()-starttime>3000){
  ...
  if (whitenum==2){
    ...
    firstwhites[0];//onlywhite
    char idx=(que+2)%2;
    if (preError[idx]>0){
      for(int i=firstwhites[0];i<8;++i){
      sensorValues[i]=minimum[i];
      }
    }
    else{
  ...
  int deltaError=error-preError[que];
  // for(int i=1;i<4;++i){
  //   preError[i]=preError[i-1];
  //   preDelta[i]=preDelta[i-1];
  // }
  preError[que]=error;
  preDelta[que]=deltaError;
  que=(que+1)%3;
  // float weighted=deltaError*0.35+preError[1]*0.25+preError[2]*0.2+preError[3]*0.2;
  // float weighted=(error/2+(preError[0]+preError[1]+preError[2])/4)/1.25;
  float weighted=error;
  // float Dchange=(preDelta[0]+preDelta[1]+preDelta[2])/3;

  float Dchange=0;
  // int Ichange=constrain(weighted*Kd,-base_speed*0.8,base_speed*0.8);
  change=constrain(weighted/Kp-Dchange*Kd,-base_speed,base_speed);//
